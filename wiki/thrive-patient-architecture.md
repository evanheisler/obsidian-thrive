...
`apps/patient` — Expo SDK 55 / RN 0.83 / React 19, expo-router (typed routes), ships iOS + Android + Web (Metro web, `output: 'single'`). Bundle `com.bionichealth.members`, EAS project `member-app`. **Multi-brand**: thrive + kyzatrex from one binary via `EXPO_PUBLIC_BRAND` (`app.config.ts` `brandEnvironment()`/`brandAsset()`). Ships **dark by default** (`DEFAULT_THEME_MODE`; module-pinned `colorScheme` in `theme/brand-theme-colors.ts`, BH-2370) but theme mode is now **live-switchable** (light/dark) via `ThemeModeProvider`/`useThemeMode` (theme v2, #796); light mode is in active polish (BH-3499, `log: 2026-07-23`).
...
- **The app mode must be pushed to THREE independent layers or a surface renders in the wrong mode** (`log: 2026-07-23`, BH-3499): (a) **NativeWind CSS vars** — automatic via `ThemeProvider`; (b) **native iOS window appearance** — `Appearance.setColorScheme(mode)` in `LiveThemeProvider`, else native chrome (tab-bar/header liquid-glass BlurViews, keyboard) follows the *system*, so a light-mode app on a dark device shows dark-until-scroll glass; (c) **react-navigation theme** — `ThemeProvider value={DarkTheme|DefaultTheme}` (`components/navigation-theme-sync.tsx`) around the root Stack, else Expo Router's default *light* theme creates native stack headers light and they flash to the app mode a frame after mount. Layers (b) and (c) were both missing pre-BH-3499. NativeTabs already syncs via `experimental_userInterfaceStyle: mode` per-trigger (`app/(tabs)/_layout.tsx`).
- **Text/glyphs on a `brand-default` (green) surface use `text-foreground-inverted`** — the sanctioned pair in `packages/tokens/contrast/contrast-pairs.ts` (refs: `ui/avatar`, `ui/chip` `selected-bold`). Dark `foreground-default`/`-strong` on brand-green is the recurring light-mode violation class BH-3499 swept app-wide; received-message / outline-button / light-surface siblings correctly stay dark. Stream Chat own-message text needs `myMessageTheme` (native `<Channel>`) / `--str-chat__own-message-text-color` (web) — the *base* theme colors received messages, so never blanket-swap it.
