...
metadata: 
  ...
  modified: 2026-08-05T22:24:33.442Z
---
...
**Published text never names future work at all.** "Worth its own ticket", "belongs
in a separate PR", "flagged for a follow-up", "out of scope here", and citing a cost
to justify *not* doing something are banned in every framing. This is the version
that keeps slipping: an agent reads "flagging a follow-up" as recording a fact rather
than making a decision, so a ban on *decisions* never fires. It recurred three times
in one session (#953's verification comment, #959's lint note, #960's holdout reply)
even with the decision rule in the dispatch prompt. Present facts are fine
("`providers/` is outside the lint globs, which is why nothing caught this"); the
moment it becomes what someone should do about that, cut it. Work beyond the change
is either a ticket Evan authors or it does not exist.
