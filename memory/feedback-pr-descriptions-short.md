...
metadata: 
  ...
  modified: 2026-08-14T16:56:35.270Z
---
...
**No hard-wrapped lines — fourth correction (2026-08-14, PR #1063):** the rule already lives in the repo's write-pr skill (`.claude/skills/write-pr/SKILL.md` §2 "Draft the body": one paragraph per line, never hard-wrap, the tell is any paragraph spanning more than one source line). It kept recurring because my dispatch prompts hand executors finished body text or a custom body spec instead of routing them through the skill — the skill's rule never executes. Fix at the source: dispatch prompts never carry PR body text; they say "compose the body with the write-pr skill." My own `gh pr create/edit --body-file` files follow write-pr §2 directly.
