...
metadata: 
  ...
  modified: 2026-08-10T17:39:08.661Z
---
...
Third instance, vendor-API jargon (2026-08-10): asked "Rebuild the walk on the `useStateForPath` spine?" — Evan: "That question is nonsense but it sounds like a design flaw that needs to be patched." A vendor hook name + my own shorthand ("walk", "spine") is the same failure as an unexpanded acronym. The ask should have been: "replace how the hook finds its enclosing tab — read React Navigation's per-screen ancestry data instead of parent state that isn't populated on first render?"
