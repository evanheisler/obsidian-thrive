...
metadata: 
  ...
  modified: 2026-08-10T19:54:45.436Z
---
...
**How to apply:** In every reorg/story dispatch prompt, state the objective, not just the shape rules: "entries are earned — if the component is a wrapper/parametrization of an existing entry, fold or delete its story and say so in your return; do not converge it." When a placement question produces friction, first ask whether the subject should exist at all. "Reviewable" for a loop PR means it abides by the CURRENT skill text — a skill update retroactively un-readies open PRs until they're re-derived against it; the skill settles test-derivable verdicts, so don't route those to Evan per instance. Related: [[feedback-current-shape-is-not-a-requirement]], [[feedback-dry-targets-functional-identity-not-ui]], [[rules-encode-the-test-not-verdicts]].
