...
metadata: 
  ...
  modified: 2026-08-10T17:47:36.814Z
---
...
**SIXTH violation (2026-08-10):** presented Evan a fork — "#1010 is open; let it own the fix, or #996 keeps it?" — built on a subagent evaluation from days earlier. #1010 had merged 90 minutes before I asked ("1010 is already merged dipshit. Why is this a question"). The decision evaporated on fetch: main already carried the fix, the rebase absorbs it mechanically. **A decision prompt IS a status assertion — every PR/issue lifecycle fact inside a question's premises gets the same this-turn `gh`/`linear` fetch as a status table. A fork whose premises weren't re-fetched this turn may not be asked.** Subagent returns age like any other cache.
