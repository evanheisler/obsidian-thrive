...
metadata: 
  ...
  modified: 2026-07-31T17:11:48.525Z
---

Do NOT re-request bot reviews by toggling the `claude-review` label after a push or fix (the `codex-review` label is dead as of 2026-07-31 — never add it at all; see [[thrive-bot-reviews-label-triggered]]). Bot review runs **once** per PR (at creation). When a fix addresses a finding: **push + reply `Addressed in <sha>` in-thread + resolve the thread** — that is the whole loop. Do NOT re-toggle labels to re-run the bots to "confirm" the fix. A reviewer sees the fix from the diff + the reply; no bot reconfirmation is needed. CI checks (tests/lint) re-run on push automatically and unavoidably — that is fine; the *bot review* is the expensive thing not to re-trigger.
